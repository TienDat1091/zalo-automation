# virtual_sidebar.py - Virtual Scrolling Sidebar (Fix for 1000+ friends)
# Implements lazy rendering to prevent UI freeze
...
class VirtualSidebar(ctk.CTkFrame):
    ...
    def __init__(self, parent, app):
        ...
        self.item_height = 62  # Height of each friend item
        self.visible_items = 10  # Approximate items visible at once
        self.buffer_items = 5  # Extra items to render for smooth scrolling
        self.start_idx = 0
        ...
        self.friends_scroll.bind("<MouseWheel>", self._on_scroll)
        self.friends_scroll.bind("<Button-4>", self._on_scroll)
        self.friends_scroll.bind("<Button-5>", self._on_scroll)
    ...
    def update_friends(self, friends):
        ...
        self.filtered_friends = friends.copy()
        self.filtered_friends.sort(key=lambda f: f.get('lastTimestamp', 0), reverse=True)
        self.start_idx = 0
    ...
    def _render_visible_items(self):
        ...
        end_idx = min(self.start_idx + self.visible_items + self.buffer_items, len(items))
        port = self.app.config_port
        ...
        print(f"📊 Rendering items {self.start_idx}-{end_idx} of {len(items)}")
    ...
    def _on_scroll(self, event=None):
        ...
        # Update start_idx based on scroll position
        # This is approximate - can be refined with actual scroll offset
        scroll_frame = self.friends_scroll
        if hasattr(scroll_frame, '_parent_canvas'):
            try:
                # Get scroll position (0.0 to 1.0)
                scroll_pos = scroll_frame._parent_canvas.yview()[0]
                max_items = len(self.filtered_friends)
                self.start_idx = max(0, int(scroll_pos * (max_items - self.visible_items)))
                self._render_visible_items()
            except Exception:
                pass
    ...
    def _on_search(self, event=None):
        ...
        if not query:
            self.filtered_friends = self.all_friends.copy()
        else:
            self.filtered_friends = [f for f in self.all_friends
